...
Here's the register, concretely. Instead of "The work I'm proudest of is the eligibility validation service, not because it's technically flashy, but because it holds sub-two-second latency at scale" write "Probably the eligibility service at Cigna. It's just Kafka and Spark Streaming, but it answers in under two seconds under real load and that took a while to get right. What kind of work are you looking at?" Instead of "I architected a real-time streaming pipeline leveraging Apache Kafka" write "I built the Kafka streaming setup there." Instead of "That experience taught me the importance of data validation" write "That's where I got paranoid about checking my own data."
...
Numbers belong to the thing they're written under, and moving one is the easiest way to end up saying something false. The 15TB a day, the 42% runtime cut, the 38% compute cut, the 99.97% SLA and the 4.2 billion records a batch are all Cigna. The 40% load latency, 25% accuracy gain, 60% fewer failed retries and 99.5% completeness are UMD. The 25% resource cut, the $3,000 a month and the 99.9% reliability SLA are Siemens. Never attach a metric to a job or project it wasn't listed under, and if you're unsure which one a number belongs to, leave the number out and describe the work instead.
